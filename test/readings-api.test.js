...
import {onRequestGet as listReadings} from '../functions/api/trips/[tripId]/readings.js';
import {onRequestGet as readReading,onRequestPut as putReading,onRequestDelete as deleteReading} from '../functions/api/trips/[tripId]/readings/[readingId].js';
import {onRequestGet as readPublicReading} from '../functions/api/public/trips/[token]/readings/[readingId].js';
...
test('authenticated readings API writes markdown outside the trips JSON document',async()=>{
  ...
  const listed=await (await listReadings({params:{tripId:'one'},env})).json();
  ...
  const read=await (await readReading({params:{tripId:'one',readingId:'jiaxing-nanhu'},env})).json();
  ...
  const missingRead=await readReading({params:{tripId:'one',readingId:'nope'},env});
  ...
  assert.equal((await (await readReading({params:{tripId:'one',readingId:'jiaxing-nanhu'},env})).json()).markdown,'# 修订');

  const removed=await deleteReading({params:{tripId:'one',readingId:'jiaxing-nanhu'},env});
  assert.equal(removed.status,200);
  assert.equal((await readReading({params:{tripId:'one',readingId:'jiaxing-nanhu'},env})).status,404);
});
...
test('shared trip can read a stored reading body without login and cannot see another trip',async()=>{
  ...
  const publicRead=await readPublicReading({params:{token,readingId:'r1'},env});
  assert.equal(publicRead.status,200);
  assert.equal((await publicRead.json()).markdown,'# 公开正文');
  assert.equal((await readPublicReading({params:{token,readingId:'secret'},env})).status,404);
  assert.equal((await readPublicReading({params:{token:'a'.repeat(43),readingId:'r1'},env})).status,404);
});
