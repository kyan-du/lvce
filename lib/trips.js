...
export const ZHANGJIAJIE_ITINERARY_MIGRATION='zhangjiajie-itinerary-20260806-v11-ctrip-bookings';
...
{id:'ticket-tianmen-20260808',type:'ticket',title:'天门山A线成人票',match:'天门山A线',platform:'携程',orderNumber:'1128148375850216',credential:'2026-08-08 07:00-08:00；成人2人：杜万、胡丽霞；天门山索道下站A线入口；证件原件+电子凭证或纸质凭证；儿童票未见截图，状态未确认',amount:'¥576',privateNotes:'辅助码：ANWSK26080853374340612；AWSK26080853391211161',openWith:'携程App→我的→全部订单'},
...
function migrateZhangjiajieTrip(trip){
  ...
  if(!Array.isArray(next.bookingDetails)){next.bookingDetails=[];changed=true}
  for(const detail of ZHANGJIAJIE_BOOKING_DETAILS){
    const exists=next.bookingDetails.some(item=>item?.id===detail.id);
    const hasSource=detail.type==='transport'?next.transport?.some(row=>has(row)(detail.match)):(next.tickets?.some(row=>has(row)(detail.match))||next.itinerary?.some(row=>has(row)(detail.match)));
    if(!exists&&hasSource){next.bookingDetails.push(structuredClone(detail));changed=true}
  }
  if(!Array.isArray(next.itineraryLinks)){next.itineraryLinks=[];changed=true}
  for(const link of ZHANGJIAJIE_ITINERARY_LINKS){
    const hasRow=next.itinerary.some(row=>row?.[0]===link.date&&row?.[2]===link.activity);
    const exists=next.itineraryLinks.some(item=>item?.date===link.date&&item?.activity===link.activity);
    if(hasRow&&!exists){next.itineraryLinks.push({...link});changed=true}
  }
