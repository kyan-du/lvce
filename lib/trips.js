...
function migrateZhangjiajieTrip(trip){
  ...
  rows=rows.filter(row=>!(Array.isArray(row)&&((row[0]==='2026-08-07'&&(has(row)('金鞭溪')||has(row)('G9679')||has(row)('芙蓉镇')))||(row[0]==='2026-08-08'&&(has(row)('C7769')||has(row)('猛洞河')))||(row[0]==='2026-08-09'&&(has(row)('开福寺')||String(row[2]||'').startsWith('C7947')||String(row[2]||'').startsWith('G206')||has(row)('金鞭溪'))))));
  for(const row of [ZHANGJIAJIE_MENGDONG_ROW,ZHANGJIAJIE_JINQI_ROW,ZHANGJIAJIE_TIANMEN_ROW,ZHANGJIAJIE_JINBIANXI_ROW,ZHANGJIAJIE_JINBIANXI_TRANSFER_ROW,ZHANGJIAJIE_C7947_ITINERARY,ZHANGJIAJIE_G206_ITINERARY])if(!rows.some(existing=>sameRow(existing,row)))rows.push(cloneRow(row));
