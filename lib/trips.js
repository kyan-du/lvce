...
export const QIANTANG_TRIP_MIGRATION='qiantang-trip-20260920-v1-bookings';
...
export function buildQiantangTrip(){
  return {id:'qiantang',name:'钱江潮',meta:'2026年9月 · 嘉兴／海宁',categories:[],itinerary:QIANTANG_ITINERARY_ROWS.map(cloneRow),transport:QIANTANG_TRANSPORT_ROWS.map(cloneRow),hotels:QIANTANG_HOTEL_ROWS.map(cloneRow),tickets:[],emergency:QIANTANG_EMERGENCY.map(cloneRow),tour:[]};
}
function migrateQiantangTrip(trip){
  if(trip?.id!=='qiantang')return {trip,changed:false};
  const originalJson=JSON.stringify(trip);
  const next={...trip,categories:Array.isArray(trip.categories)?trip.categories:[],tickets:Array.isArray(trip.tickets)?trip.tickets.map(row=>Array.isArray(row)?row.slice():row):[],tour:Array.isArray(trip.tour)?trip.tour.map(row=>Array.isArray(row)?row.slice():row):[],transport:Array.isArray(trip.transport)?trip.transport.map(row=>Array.isArray(row)?row.slice():row):[],hotels:Array.isArray(trip.hotels)?trip.hotels.map(row=>Array.isArray(row)?row.slice():row):[],itinerary:Array.isArray(trip.itinerary)?trip.itinerary.map(row=>Array.isArray(row)?row.slice():row):[],emergency:Array.isArray(trip.emergency)?trip.emergency.map(row=>Array.isArray(row)?row.slice():row):[]};
