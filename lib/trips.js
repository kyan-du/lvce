...
export const QIANTANG_TRIP_MIGRATION='qiantang-trip-20260925-v6-itinerary-db-owned';
...
function sortQiantangItinerary(rows){
  return rows.map((row,index)=>({row,index})).sort((a,b)=>{
    ...
    if(date)return date;
    const time=String(a.row?.[1]||'').localeCompare(String(b.row?.[1]||''));
    if(time)return time;
    return a.index-b.index;
  }).map(item=>item.row);
}
export function buildQiantangTrip(){
  return {id:'qiantang',name:'钱江潮',meta:'2026年9月 · 嘉兴／海宁',categories:cloneCategories(QIANTANG_PACKING),itinerary:[],transport:QIANTANG_TRANSPORT_ROWS.map(cloneRow),hotels:QIANTANG_HOTEL_ROWS.map(cloneRow),tickets:[],emergency:QIANTANG_EMERGENCY.map(cloneRow),tour:[],readings:cloneReadings(QIANTANG_READINGS)};
}
function migrateQiantangTrip(trip){
  ...
  // Itinerary content is database-owned: do not upsert/overwrite from seed rows.
