import {migrateTripDocument,validateDocument,MAX_BYTES} from '../../lib/trips.js';
const headers={'Cache-Control':'no-store'};
export async function onRequestGet({env}){let row=await env.DB.prepare("SELECT data, updated_at, version FROM documents WHERE id = 'trips'").first();let data=row?JSON.parse(row.data):null,version=row?.version||0,updatedAt=row?.updated_at;if(row){const migrated=migrateTripDocument(data);if(migrated.changed){const json=JSON.stringify(migrated.data);const result=await env.DB.prepare("UPDATE documents SET data=?, updated_at=datetime('now'), version=version+1 WHERE id='trips' AND version=?").bind(json,row.version).run();row=await env.DB.prepare("SELECT data, updated_at, version FROM documents WHERE id = 'trips'").first();data=row?JSON.parse(row.data):null;version=row?.version||0;updatedAt=row?.updated_at;if(result.meta.changes!==1&&data){const current=migrateTripDocument(data);data=current.data}}}let shares={};try{let result=await env.DB.prepare('SELECT trip_id FROM public_trip_shares').all();shares=Object.fromEntries((result.results||[]).map(r=>[r.trip_id,true]))}catch{}return Response.json(row?{data,updatedAt,version,shares}:{data:null,version:0,shares:{}},{headers})}
export async function onRequestPut({request,env}){
 ...
 data=migrateTripDocument(data).data;
